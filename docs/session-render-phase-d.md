...
## 6. 实施结果（2026-09-18，全部交付）

四项均已落地，全量测试 749 项（747 pass / 0 fail / 2 skip），浏览器验收 `render-capacity-ui.py` 7/7、`smooth-stream-browser.py` 16/16 全 PASS。

### 最终形态

| 子项 | 落点 | 关键参数 |
| --- | --- | --- |
| D1 有界 DOM 窗口 | `trimMountedWindow()`，`prependHistory` 尾部调用 | 上限 300 条记录，只裁前插方向 |
| D2 页级渲染缓存 | `markdown.js` `createMarkdownPageCache()` + 快照路径挂键 | LRU 300 条 / 4MiB，键 = 会话+修订号+条目 |
| D3 流式尾部增量 | `maskCodeCached` 等增量掩码 + 思考面板统一节流 | 段边界前缀缓存 + 尾窗重扫 |
| D4 发送序列化 Worker | `serializeOffThread()`，内联 Worker 源码 | 阈值 1MiB，无 Worker 退化同步 |

设计上与原计划的两处偏离：

- **D1 只裁前插方向，不裁顶部**。向下（prefetchForward / 下翻）本来就是整页替换，天然有界；顶部裁剪需要反向滚动锚点补偿，收益不足而风险高。代价是**实时跟随新消息的方向仍无上限**，长时间挂机的会话 DOM 会继续增长——已知边界，未在本轮解决。
- **被裁端回载不沿用 after 游标**，改以「被裁第一条」为 target 整页重挂。游标由服务端签发不可伪造，沿用旧 nextCursor 会跳过被裁区间形成页隙。

### 真实浏览器暴露的两个缺陷（jsdom 覆盖不到）

500 条混合消息（用户 / Markdown 回复 / 思考+工具调用 / 工具结果）的真实验收，抓出两个纯文本用例结构上无法触发的问题：

1. **混合会话完全不裁剪**。`trimMountedWindow` 原本遇到「没有独立 DOM 节点」的条目就 `break`，而工具结果记录的内容渲染进前一条助手卡的过程组、本身不拥有节点。尾部一旦是工具结果，裁剪立刻停在 0 条，500 条记录全部挂载（375 个消息节点）。修复：这类条目连同它所属的主卡一起纳入丢弃集合，并把裁剪边界对齐到拥有自己节点的条目上，保证丢弃是 DOM 完整的；压缩共享节点仍是停止点。jsdom 用例只用纯文本记录，每条都有独立节点，所以一路通过。
2. **滚底回载只能靠手点按钮**。`prefetchForward` 在 `!nextCursor` 时直接返回，而从末页往上翻时 nextCursor 恒为 null（前插只更新 start 与 prevCursor）。分页按钮的处理函数考虑了 `trimmedForwardBoundary`，滚动路径却没有。修复：boundary 存在时同样视作有下页。

附带效果：裁剪未生效时分页条位置文本恒为空（`page.nextCursor || trimmedForwardBoundary` 都为假），修复后正常显示 `1–300 / 共 500 条`。

### 实测数据（4× CPU 节流，500 条混合会话）

- attach → 就绪 + 连续前插到最早页：约 1.8s，窗口稳定在 300 条记录 / 225 个消息节点，DOM 约 8.5k 节点。
- 滚底 target 重挂：窗口整体替换为 120 条记录 / 90 个消息节点，DOM 降到约 4k，被裁边界条目回载在场。
- 交互延迟：滚动 p95 约 0.1ms，按键 p95 约 0.2ms。
- 全部 details 展开后消息节点数仍在窗口上限内（懒渲染内容进 DOM 后不破界）。

### 验收口径踩坑（复现时注意）

- 工具结果文本是懒渲染的，未展开时不在 DOM，不能用它做「内容在场」断言。
- `scroll` 事件异步派发（下一帧前合并），同步循环结束就摘监听器会导致计数恒 0。
- 程序化赋值 `scrollTop` 在部分情形下不触发监听器，需手动派发一次 `scroll`；且相对位移在已到窗口顶部时恒为 0，压测每轮必须真的换位置。
- 注入分页 stub 时，JS 对象字面量里后面的同名键会覆盖前面的——`snapshot` 必须原地替换，不能在头部追加。
