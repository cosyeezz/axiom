...
- 主会话用柔紫色 `SUBAGENT` 卡片显示子任务状态和简短标题；点击打开独立原生浮层，不在对话中堆叠详情，也不因新任务自动弹出抢焦点。浮层以右侧会话内容区居中、宽度保留 880px 上限，位置和遮罩避开左侧配置栏与顶部会话标题；侧栏收起、窗口缩放和手机布局同步适配。
...
- 等待模型响应显示「连接中…」和统一 20px 线条 SVG；收到思考后只保留一条「思考中… / 思考过程」入口，右侧「查看 / 收起」，不再重复状态行。连续没有正文、没有工具调用的思考合并展示，不堆叠空白 AXIOM 卡片和重复模型用量。
- 工具行按「语义图标 → 中文动作名 → 操作对象 → 状态 → 详情」排列。文件、命令、搜索等各有固定 SVG，完成后不替换成小对勾；工作空间内路径显示相对路径，长摘要省略，悬停保留完整值。手机将对象移到第二行；未知工具保留原名。
...
- 思考中使用灯泡外旋转环，连接中使用旋转 SVG，工具执行中保留独立状态转圈；子代理启动中/运行中的卡片、浮层标题与输入区摘要均有清晰转圈。结束后停止，系统开启「减少动态效果」时静止但仍保留状态文字。
...
视觉回归：先运行 `node tests/conversation-preview.mjs`，再用已有 Python Playwright + Chromium 环境执行 `python tests/conversation-ui.py [截图输出目录]`。测试使用静态样例，不读取用户数据、不调用模型；覆盖 1440/390/320px 配色、字号、思考字重、真实转圈/减少动态效果、长内容吸顶收起、无内部纵向滚动与子代理定位。
...
会话里的原生 `<skill>` 消息块显示为默认折叠的 `[skill] 名称` 标识，与 Pi 终端的折叠展示一致；点击展开路径和经过 Markdown 安全渲染的正文，skill 块放在对应用户消息前面，与用户气泡平级，不嵌套；纯 skill 消息不显示空气泡，历史恢复也适用。该标识表示技能正文进入消息，不表示模型已完成执行。
