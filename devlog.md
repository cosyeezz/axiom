...
## 2026-09-24 目标图标替换与同行布局验证

- 原因：用户指定使用 `素材/goal.svg`，中心改为红色，并与相邻及同行长控件协调。
- 修改：保留素材填充轮廓，分离中性外环与红色内环/箭头；16px 原稿等比映射至 24px 网格中的 18px 图形区域，沿用 20px 图标外框和 28px 居中按钮，不改操作行为。红色复用既有目标配色 token，保留主题和 hover 适配。
- 验证：app/icons/composer-controls 共 12 项测试通过；Chromium 深浅主题与 390px 窄屏回归通过，检查实际图形边界、中心、颜色、溢出，以及长上下文标签和计时器同排不重叠；检查浅色截图。合并前同步 origin/master，基线未变化。
- 文件：public/icons.js、public/style.css、tests/composer-icons-ui.py、README.md、devlog.md。
...
- 修改：新增构造方案，记录唯一 Axiom 入口、常用动作说明常驻、低频能力按需发现、后台校验执行及委派示例。
- 决策：不动态追加原生 schema，不常驻完整能力目录；区分模型侧调用说明与后台执行契约，明确缓存和参数可靠性的取舍、一期边界及待定事项。纯讨论文档不调整 README，不修改原记忆方案或实现代码。
- 验证：文档内容与讨论共识核对，检查 Markdown 示例 JSON、关联文档路径及暂存差异；仅文档变更，不运行应用测试。初次远端同步因 SSH 认证失败中止，经用户要求重试后成功。
- 文件：docs/Axiom-universal-tool-design-2026-09-24.md、devlog.md。

## 2026-09-24 子代理对话与会话模型热切换

- 原因：子代理详情只有查看和停止入口，模型修改只能影响后续委派，不能在执行过程中调整后续请求。
- 修改：详情增加独立对话输入、steer/followUp 发送及模型收藏选择菜单；新增严格校验的 task.append 路由并复用既有追加/历史续接。模型选择沿用会话 subagentModel 持久化，更新活动子代理并在首次请求前补齐创建期间的配置变更；不打断已发出的请求。
- 决策：模型设置属于当前主会话，不新增任务独立覆盖或修改全局默认；跟随主代理时主模型修改同步生效。保留原有停止、历史与执行预算约束，发送失败不清空草稿。UI 复用现有 --line/--muted 和模型菜单，采用 8/12/24px 间距，不新增装饰色。
- 验证：全量 886 项（884 通过、2 跳过、0 失败）；覆盖协议、前端发送与选模、活动任务热更新、创建期间竞态、会话模型持久化/重载、停止守卫、鉴权挂起时有界停止及主会话通知冻结；同步 origin/master 后复验通过。未调用付费真实模型，未做真实浏览器视觉验收。
- 文件：public/app.js、public/index.html、public/style.css、src/protocol.js、src/server.js、src/sessions.js、src/tasks.js、tests/app.test.js、tests/tasks.test.js、tests/session-persistence.test.js、README.md、devlog.md。

## 2026-09-24 记忆查询机制讨论共识独立归档

- 原因：用户要求先单独固化本轮讨论，随后继续细化；不修改原记忆方案或运行时代码。
- 修改：新增按需记忆咨询讨论文档，记录主代理与专用记忆 Pi 的职责、模型自主查询、汇报前核对要求、skill 的放置位置及一期暂不接 Jev/Laya 的方向。
- 决策：不绑定 Todo 逐单元查询，不以全量预取或复杂缓存为前提；优先把检索流程写入记忆代理专用系统提示词，接口、生命周期和后台学习保留为待讨论项。仅新增讨论资料，不变更功能或仓库结构，不调整 README；不纳入主工作区已有未提交文件。
- 验证：文档差异及空白检查；仅变更本讨论文档与本记录，无运行时代码变更，不运行应用测试。
- 文件：docs/Axiom-memory-query-discussion-2026-09-24.md、devlog.md。

## 2026-09-23 修正目标箭头与工具栏视觉尺寸

- 原因：用户指出目标箭头朝外、填色与线描风格不统一，同排图形大小和对齐不一致；上一版测试只检查 SVG 外框及相对按钮中心，未覆盖实际图形边界和全排中心线。
- 修改：目标改为红色空心靶环，右上留出箭道，蓝色箭头从 (21,3) 指向 (12,12)；移除实心靶面及多余圆心。添加图标宽度与信息图标圆径收至 18，工具栏五个图形的边界均为 [3,21]、中心 (12,12)，保留 20px 外框及统一描边。
- 决策：保留用户要求的目标彩色，但以线描统一视觉重量；菜单与文字操作按钮仍为 16px，不混同于上排图标尺寸。
- 验证：Chromium 明暗主题与 390px 移动端检查实际 getBBox、全排中心线、外框尺寸、无填色、箭头方向和颜色；截图复核通过。全量 963 项，961 通过、2 跳过、0 失败；同步 origin/master 无新变更，diff 检查通过。自动生成索引不纳入提交。
- 文件：public/icons.js、public/style.css、tests/composer-icons-ui.py、README.md、devlog.md。

## 2026-09-23 素材操作图标统一重绘

- 原因：用户要求使用素材中的添加、目标、发送、停止和强制停止图形，统一风格并指定语义配色。
- 修改：基于素材轮廓重绘 24px 网格、1.75px 圆角描边 SVG；列表加号替换回形针、红白靶环配蓝色命中箭头、蓝色纸飞机、深红圆角方块与警告三角。色彩由 CSS 控制，不影响共享 actionIcon 或操作行为。
- 决策：按用户要求对默认单强调色规范作局部例外；保留不同形状和标签，不以颜色作为唯一识别依据。运行时不依赖未跟踪的素材目录。
- 验证：定向测试 12/12；全量 963 项，961 通过、2 跳过、0 失败；Chromium 明暗主题与移动端回归通过，检查颜色、目标 hover、尺寸和居中。已同步远端 master，无新增变更；diff 检查通过。worktree 初次缺依赖，链接现有 node_modules 后验证成功；自动生成索引不纳入提交。
- 文件：public/icons.js、public/style.css、tests/icons.test.js、tests/composer-icons-ui.py、README.md、devlog.md。
...
- 决策与修改：`public/app.js` 取消拆组及模型统计搬运，安全点留在所属记录内部，随调用组展开；保留原统计节点在消息尾部。`public/style.css` 将安全点占位及底部间距各从 4px 收紧到 2px，触屏仍保留 40px 点击区域，移除旧 boundary 样式。
- 验证：`tests/app.test.js` 覆盖内部标识、统计归属、重复渲染及真实 refreshCallGroups 连续两次执行后仍为单组；全量 961 项（959 通过、2 跳过、0 失败），diff 检查通过，远端 master 仍为验证基线 ebd9ac3。未进行真实浏览器像素测量。
- 文件：`public/app.js`、`public/style.css`、`tests/app.test.js`、`README.md`、`devlog.md`。

## 2026-09-23 安全点标识线的间距与用量归属

- 原因：安全点按钮继承全局最小高度，令 Fork／回退标识与相邻消息间距显著增大；工具结果锚点与助手消息统计行分属不同节点，需要按消息归属定位安全点。
- 修改与决策：`public/style.css` 将安全点自身收窄为 4px，按钮绝对定位在原有消息行间；触屏保持 40px 点击区域，菜单浮层不参与排版。`public/app.js` 显式保存工具所属助手消息，在中间安全点拆分调用组，将原模型统计节点移到组外、标识线上方；重建入口前先归位，避免折叠隐藏、重复统计或移除入口时丢失节点。`tests/app.test.js` 验证布局与 DOM 顺序；`README.md` 同步交互说明。
- 验证：全量 961 项（959 通过、2 跳过、0 失败）；DOM 顺序覆盖卡内工具、卡后调用组、折叠组中间拆分和重复渲染归位；CSS 断言覆盖桌面紧凑间距及触屏点击区域。未做真实浏览器像素测量。获取远端 master 确认为基线 `47e2004`。
- 文件：`public/style.css`、`public/app.js`、`tests/app.test.js`、`README.md`、`devlog.md`。
...
- 修复：完成／暂停状态不注入读命令；同运行同版本同规范化查询第3次读取持久暂停并requestSafeStop，冻结通知自动唤醒。空清单也可因循环持久暂停；普通停止空清单保持既有语义。版本变化、非Todo工具结束、新运行重置计数，分页查询独立计数。
- 验证：全量935项，933通过、2跳过、0失败；真实SDK配本地SSE模拟模型重复调用，确认仅3次请求且第4次不发送，工具结果保留；SQLite暂停恢复与Sessions通知阻断回归通过。未调用真实供应商，不重启服务。
- 文件：src/todo.js、sessions.js；tests/todo.test.js、goal-sessions.test.js、safe-stop.test.js；README、执行规范和devlog。
...
- 实现：独立工作/收尾/总结时间预算，取消先停止在飞工作再禁工具总结；立即结束仍交付系统事实报告。执行标识隔离续接，终态 append 保留历史，展示累计耗时及清理确认状态。
- 命令策略：bash/powershell 未填 timeout 注入 180 秒；显式正数尊重，600 秒仅建议；主代理 -1 转省略、子代理禁止无限。记录有效时限及来源，超时不自动重试。
- 配置与 UI：默认 600/180/60 秒，SQLite 会话级预算快照向后兼容；任务卡可停止并总结/立即结束、填写理由，工具记录显示耗时与时限；活动状态守卫覆盖收尾/停止/总结。
- 竞态与持久化：启动取消迟到清理回写确认；停机中断保留恢复状态；停止前未消费指令保留但不擅自重启，显式排队追问等待通知送达。历次结果凭证保留，通知按 resultId 确认；SQLite 待通知列聚合所有执行，重启幂等补发，复制/退出目标同步抑制。
- 验证：全量 923 项，921 通过、2 跳过、0 失败（约 66 秒）；真实 SDK + 本地模型桩验证总结禁工具及恢复、实际 bash 默认超时事件和超时反馈；UI DOM 回归覆盖停止模式/理由、重连耗时、预算表单，未执行真实供应商调用或真实浏览器人工验收。修复旧测试提前 finish 与漏清理导致的挂起。git diff --check 通过。
- 索引：模块登记与知识记录同步；索引重建 276 文件、0 未登记，生成 INDEX.md 不提交以保留主工作区已有修改。
- 涉及：src/tasks.js、task-execution.js、tool-execution.js、task-budget.js、pi.js、prompts.js、tools.js、protocol.js、sessions.js、session-store.js、server.js、goal.js；public/app.js、index.html、style.css；相关 tests、README、执行规范。
...
- 实现：新增 `src/native-summary.js` 调 SDK 原生 compact，首次／连续／split-turn 附最小摘录要求，观察实际请求与完整输出，不采集鉴权参数。`src/compaction-excerpt.js` 逐字匹配内联证据并替换 `[消息:ID]`，继承引用验权；`history-tools.js`／`pi.js` 主会话仅暴露精确 history_read，保留旧 ref 读取兼容。
- 展示：`public/compaction-view.js`、`app.js`、`style.css` 增加按需尝试详情、Markdown／原文／复制、完整流与原始／处理后摘要。`protocol.js`／`server.js`／`sessions.js` 接详情接口、冷恢复与会话删除清理。过程记录与无内容诊断分离，最近五次持久保存，已接收的请求正文不重复拉取。
- 安全：保留归档覆盖、分支失效、持久确认与窗口检查。原生回退仍限空闲手动同步，独立记录尝试；自动／异步回退统一尚未交付，不宣称完整定稿已全部验收。
- 文档与索引：同步 README、定稿实施状态以及 `.pi/skills/codebase-map/scripts/reindex.mjs` 模块登记，生成索引随测试重建但不纳入本次提交，避免覆盖主工作目录已有索引修改。
- 测试：最终全量 899 项，897 通过、2 跳过、0 失败；新增原生首次／连续／split、观察流终态／取消、摘录、精确读取与安全 Markdown 测试。均为本地测试／模拟供应商，未执行真实供应商两轮持久应用验收，未重启服务。独立审查后补齐实际摘要请求预算检查、新证据范围限制、回退取消、XML 可见性、完整请求结构、版本短路、失败恢复及回到底部；补充回退取消回归后，最终全量 899 项（897 通过、2 跳过）；确认 SDK abort() 会取消原生 compaction。详细请求记录按产品定稿保留，不采纳删除全部正文的审查建议。

## 2026-09-22 压缩简化与可观察展示定稿（00:46 -0700）

- 原因：用户连续讨论后确认最小增强方案，并要求提示词与输出不再堆成难以分析的文本块；本轮只定稿文档，不实施运行逻辑。
- 决策：复用原生摘要规则，不给输入加消息编号，不输出独立 facts 数组；模型自选重要内容附内联原文，程序精确匹配替换原始消息 ID。原生已有工具结果截断明确区分 OP；旧引用随有效结论继承，不要求逐消息覆盖。读取工具仅按 ID＋关键词精确定位，不跨历史搜索、不做会话 UI 分页。
- 展示：实际请求、完整输出流、引用处理和最终结果分区；原生回退独立尝试留档，安全 Markdown 预览／原文切换与复制，稳定滚动，复用 Linear 双主题及既有代码块组件。不展示隐藏推理，不采集鉴权信息，不以展示替代原始请求记录。
- 文件：重写 `docs/compaction-simplification-plan.md`，同步 `README.md` 入口并标注当前旧路径，更新 `devlog.md`。保留旧实验现场，明确首次／连续／split-turn接线待验证，不能把定稿当作已实现。
- 验证：仅文档修改，检查三文件范围与 `git diff --check`；未运行模型请求或代码测试。

## 2026-09-21 手动同步压缩的一次原生回退

- 原因：用户要求先启用最简单的原生降级，重启后自行触发压缩，暂不继续复杂方案；分支 `feat/native-compaction-fallback`，不合并旧严格工具实验。
- 内容：仅手动同步且会话空闲时，增强生成 promise 失败后调用一次 SDK `session.compact()`；保留取消、配置代际、stop 类错误、提交健康与归档屏障，应用错误不回退。原生回退不加增强指令，不改主模型或用户配置；状态区分回退及未做 facts 核验。原生自动压缩仍关闭，不会递归进入增强链。
- 文件：`src/compaction.js`、`src/pi.js`、`tests/compaction.test.js`、`README.md`、`devlog.md`。
- 验证：定向50项通过，含真实 SDK + 本地模拟 HTTP 的原生 compaction 记录追加与事件测试、生成失败只回退一次、归档失败不回退、取消不回退、异步不回退及原生失败不循环。全量 `npm test` 888项：886通过、2跳过、0失败；`git diff --check` 通过。真实供应商触发由用户重启后验收，本次不调用真实模型、不擅自重启服务。

## 2026-09-21 压缩简化方案与新会话交接

- 内容：新增 `docs/compaction-simplification-plan.md`，收敛为原生压缩基座、完整原文独立归档、非阻塞展示元数据、简单 facts 精确匹配和一次可观察原生回退。
- 原因：用户认为结构化任务状态方案过于复杂，要求先生成方案，再新开会话讨论；本次不实施压缩改动，不合并未验收实验。
- 决策：方案标注原生接线仍需核实，明确回退不绕过取消／归档失败／提交不确定，区分引文存在与语义正确，并记录旧工作树与未跟踪参考文档的位置。
- 涉及文件：`docs/compaction-simplification-plan.md`、`README.md`、`devlog.md`。
- 验证：仅文档改动，检查交接信息、文档链接及 `git diff --check`；不运行模型请求或代码测试，不宣称压缩已恢复。

## 2026-09-21 鹈鹕海风骑行 HTML

- 内容：新增可直接打开的独立 SVG 2D 动画，白色鹈鹕骑珊瑚红自行车，包含踏板与车轮转动、腿部联动、围巾摆动和海岸视差，以及暂停、调速、减少动态效果支持。
- 原因与决策：用户要求生成 HTML，不测试、不使用其他模型或子代理；由当前代理直接编写，无外部资源或构建依赖，不改动现有动画页面。
- 涉及文件：`pelican-coastal-ride.html`、`README.md`、`devlog.md`。
- 核对：仅人工静态审阅和 Git 差异检查；未运行测试、浏览器或模型调用，不宣称视觉效果经过运行验证。
...
- 证据：本次失败子任务原文历史在末尾工具结果超过 5000-token 保留目标时，SDK `findCutPoint` 返回历史起点；控制器因此没有待摘要消息，最终请求前报 `WINDOW_UNSAFE`。通过只读历史回放复现，未改动用户会话与配置。
- 决策：当 SDK 切点退回起点时，按估算 token 向前寻找合法助手/用户边界，允许保留尾部超过软保留目标；继续执行既有工具配对、安全预算、摘要与持久化校验，不放宽硬限制。供应商并发超限、提示词拒绝和后台摘要超时不在此修复范围内。
- 涉及文件：`src/compaction.js`、`tests/compaction.test.js`、`README.md`、`devlog.md`。
- 验证：新增单个与并行三个大工具结果回归，验证摘要成功且调用/结果全部保留。全量 `npm test` 重跑 880 项：878 通过、2 跳过、0 失败。首次全量有 1 项既有页面异步展示断言失败，该文件独立重跑 3/3 通过，随后全量通过。`git diff --check` 通过。尚未执行真实供应商长会话端到端验收，不宣称所有子任务错误均已解决。
...
- 原因：乐观消息已上屏，但编辑器直到服务端回执才清空，配置等待及网络往返期间仍显示已提交内容。
- 决策：在第一次异步等待前同步清空文字、图片、引用和技能选择及会话草稿快照；成功回执只对账，不触碰后续草稿。确定失败仅恢复仍为空的原会话编辑器/快照，不覆盖新输入；结果未知保留未确认卡，不自动恢复待发送内容，避免误重发。
- 涉及文件：`public/app.js`、`tests/send-optimistic.test.js`、`README.md`、`devlog.md`。
- 验证：发送与页面聚焦测试 13/13 通过；全量 `npm test` 877 项，875 通过、2 跳过、0 失败。新增同步清空、配置等待、延迟成功/失败回执不覆盖相同新文本、未知状态不恢复断言；调整 runId 用例，不再用输入框清空作为回执信号。
...
- `usage-stream.js` 对本地合成的失败终态显式标记 usageKnown=false；`usage-store.js` 兼容增加 usage_known 列并在请求列表返回，避免将失败时占位零用量解释为供应商确认的零。旧记录默认未知，不反向猜测。
- 专项及全量测试通过，待最终验收闭环；不宣称未执行的真实模型质量评测通过。
...
- `raw-history.js` 重新打开时拒绝原文／控制分流不一致，恢复文件使用 0600。`tests/history-journal.test.js` 新增复制身份回归；历史专项通过，全量继续验证。

## 2026-09-21 手动压缩集成与完整产物补强

- 合入远端 `0f6db60` 手动压缩入口，保留同步／异步不同保留预算；手动入口同样经过归档屏障、窗口检查及持久提交确认，提交不确定时禁止启动。
- `raw-history.js` / `history-journal.js` / `history-tools.js` / `pi.js` 保存 SDK fullOutputPath 声明的完整文本产物，提供 artifact_0 校验分页；测试覆盖原临时文件删除后可读、归档产物缺失阻止屏障。
- 检索游标改为有界快照标识，避免把整个历史来源集塞进每页响应导致大历史无法分页；补齐搜索过滤参数。新归档权限收紧并设 16 MiB 磁盘安全余量。
- `README.md` 同步契约；持续全量复验通过（最新日志见工作区 test-results.log）。仍需完成逐项验收报告与最终集成，未推送 master。
...
## 2026-09-20 压缩与能力选择界面统一

- 时间：2026-09-20；分支 `feat/compaction-ui`。
- 内容：压缩确认框接入主题弹窗样式，固定响应式宽度与模式说明区高度；压缩方式及子代理能力模式复用自定义下拉。压缩模型增加供应商筛选与思考等级联动。
- 决策与原因：消除原生白框、系统菜单和切换尺寸跳动；模式不属于模型收藏目录，不显示收藏星标。供应商从现有完整模型键还原，不新增后端配置字段，兼容已有配置及不可用模型回显。
- 涉及文件：`public/{app.js,index.html,style.css}`、`tests/{app,compaction-ui}.test.js`、`README.md`、`devlog.md`。
- 验证：压缩交互、设置及下拉组件定向测试通过；全量 `npm test` 通过（首次服务设置异步用例失败，单独与全量复测通过）。新增 `tests/manual-compaction-ui.py`：真实 Chromium 深浅主题、1440/390/320px 下模式菜单与弹窗宽高稳定性通过。
...
- 内容：输入区新增压缩按钮、模式选择和确认；两种模式均接入自研摘要链路，手动触发不依赖自动阈值；配置增加同步 20000 / 异步 5000 tokens 保留量和用途说明，兼容旧配置。
- 决策与原因：异步不阻塞主会话且保留快照后新增内容；同步要求会话先安全停止，压缩期间锁定主会话入口，避免流式输出和工具结果写入竞态。失败/取消保留原文，复用已有进度和取消界面。
- 涉及文件：`src/{protocol,compaction,pi,sessions,server}.js`、`public/{app.js,index.html}`、压缩及会话相关测试、`README.md`、`devlog.md`。
- 验证：`npm test` 全量 843 项，841 通过、2 跳过、0 失败；新增手动同步/异步、失败保留原文、重复触发与确认/切会话防误操作测试。更新图标组与校验文案断言，workspace-tabs 测试关窗前等待后台刷新清理，避免关窗后异步回调访问已销毁 DOM。
...
- 原因：输入框上方的压缩条幅只有一行阶段文案，不能点、看不到后台摘要到底在做什么，也没有只取消本次摘要的入口（只能连带停整个会话或改配置触发取消）。
...
- 顺带修一个合入时才暴露的 master 缺陷：`3649532` 的后台装配守卫直接读 `sessions.items.has(id)`，而 `items` 是 Sessions 内部 Map。浏览器预览桩、测试桩没这个字段，`session.attach` 后的 setImmediate 里抛未捕获 TypeError 直接整进程崩：回执已发出、前端只看到 WebSocket 被拒，`tests/compaction-ui.py` 与 `tests/autoscroll-ui.py` 等所有依赖预览的验收脚本全挂。改成 `sessions.items?.has(id) === false` 才跳过（同文件 87 行早就是 `sessions.items?.values()` 这个写法）：只在确知会话消失时放弃，替身无法判断时照旧装配。`tests/session-background-server.test.js` 加一条缺 `items` 的替身用例锁住（已验过回退修改后该用例转红）。
- 文件：`src/compaction.js`、`src/pi.js`、`src/protocol.js`、`src/server.js`、`src/sessions.js`、`public/app.js`、`public/index.html`、`public/style.css`、`tests/compaction.test.js`、`tests/compaction-ui.test.js`、`tests/compaction-config.test.js`、`tests/prompt-resize.test.js`、`tests/conversation-preview.mjs`、`tests/compaction-ui.py`、`tests/session-background-server.test.js`、`README.md`、`devlog.md`。
