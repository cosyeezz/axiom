...
工作分支：feat/op-removal-compaction。初始基线 d6dc014；已集成 8594256 及其祖先。SDK 固定 0.85.1。测试使用 Node 24、真实 SDK 与本地 HTTP/SSE 假供应商，不调用付费模型。
...
| T09—T10 | custom_message 按类型归档；完整 tool fullOutputPath 产物独立保留、缺失拒绝；内嵌内容全字段保存 | 部分：多模态跨重开专项待补 |
...
| T38—T40 | 残尾隔离、来源改写/删除检测、首消息前写锁、关闭 flush | 部分：跨进程竞争和强杀专项待补 |
...
| T58 | 摘要请求接入 usage stream，关闭嵌套自动重试 | 部分：stale/rejected/cancelled 计费矩阵待补 |
