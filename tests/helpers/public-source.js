...
export const publicSource = async (...names) =>
  (await Promise.all([...new Set(names.flatMap(name => name === "app" ? ["transport", "session-cache", "session-details", "clipboard", "icons", "usage-audit", "app"] : ["file-picker", "model-picker", "model-manager", "goal", "question"].includes(name) ? ["icons", name] : [name]))].map(async (name) =>
