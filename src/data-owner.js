...
export async function claimJournalOwner(root) { return claimOwner(root, "journal"); }
async function claimOwner(root, namespace) {
  let canonical = await realpath(root);
  if (process.platform === "win32") canonical = canonical.toLowerCase();
  const key = createHash("sha256").update(namespace === "data" ? canonical : `${namespace}:${canonical}`).digest("hex").slice(0, 32);
