...
- 点击或键盘展开工具详情：edit 优先显示工具返回的实际红绿 diff，没有 diff 时明确标注为请求改动。默认窄屏使用统一视图、宽屏使用左右对比；支持手动切换并在浏览器记住选择，窄屏始终回退统一视图；write 显示写入内容（无法仅凭参数判断新建或覆盖）；bash/read 等显示参数和文本输出。内容以纯文本安全展示，展开后才生成详情，每节最多显示 60,000 字符，完整结果保留在原会话记录中。
