...
// 压缩折叠的懒加载语义：快照不下发被摘要覆盖的消息，前端点开摘要卡时按 compactionId 取原文。
...
test("快照折叠压缩段：段内主消息与其后的子代理记录都不下发，折叠前下标与总数照旧", async () => {
  await withSession(async (sessions, item, id) => {
    ...
    assert.deepEqual(state.messages.map((entry) => entry.entryId), ["m3"], "折叠段内的主消息与其子代理记录都不下发");
    // 折叠前下标必须照旧：前端按历史下标登记目标轮次锚点。
    assert.deepEqual(state.messageIndexes, [3]);
...
test("未加载会话也能按段取原文：走 JSONL 只读投影，不唤起 SDK 实例", async () => {
  ...
  try {
    ...
    // 冷快照同样按折叠投影：段内消息不下发，展开才按段取原文。
    assert.deepEqual(restored.snapshot(id).messages.map((entry) => entry.entryId), ["m2"]);
