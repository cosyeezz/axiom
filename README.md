...
- 思考、工具、Skill、重试和压缩记录隐藏默认展开三角，使用「查看 / 详情 / 收起」文字提示，仍保留原生 details 的 Enter / 空格操作与焦点提示。动态效果只用于进行中状态，尊重系统的减少动态效果设置。
...
正文与展开的思考共用 marked + DOMPurify 安全 Markdown 渲染，支持标题、粗体、斜体、列表、引用、表格与代码。正文 16px、思考 14px，以明亮重点、柔和正文和冷灰补充分层；思考不再整段强制斜体。代码块显示语言与「复制」，失败会提示手动选择复制；表格和代码可独立横向滚动并用 Tab 聚焦。复制控件在净化后生成，不允许 Markdown 注入按钮、脚本或 inline style。
...
打开 http://127.0.0.1:4319/，自动连接，无需令牌。首页采用 Linear 暗色规范（背景 #010102、面板 #0f1011、强调 #5e6ad2、边框 #23252a、正文 #d0d6e0、重点 #f7f8f8），统一文字层级、边框、留白和输入区焦点提示，提供空态及任务状态反馈。原生 HTML/CSS/JS，无前端构建，不添加字体、图标或组件依赖。
