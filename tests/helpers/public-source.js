...
export const publicSource = async (...names) =>
  (await Promise.all([...new Set(names.filter(name => name !== "goal-markers").flatMap(name => name === "app" ? ["transport", "session-cache", "session-details", "clipboard", "icons", "usage-audit", "composer-controls", "todo", "app"] : ["file-picker", "model-picker", "model-manager", "question"].includes(name) ? ["icons", name] : [name]))].map(async (name) =>
