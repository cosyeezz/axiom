...
无固定并发上限、任务超时、只读限制或结果截断。子 Agent 没有递归委派工具。并行修改同一文件可能冲突，应让主 Agent 分配不同文件，或使用不同工作目录/worktree。取消不回滚已发生的文件修改。主代理与子代理遇到模型限流、可恢复网络异常或意外中断时自动避让续跑，最多重试 45 次；间隔为 `2、2、5、5、10、10、30、60、120、240、480、960……秒`，480 秒之后翻倍，16 分钟封顶（全部耗尽纯等待约 9.3 小时）。Stop 会立即取消等待；凭证、权限、计费等永久错误不重试。可在会话配置中自定义错误词表（子串匹配、大小写不敏感）：白名单命中即强制重试（可越过内建黑名单，如供应商把瞬时故障报成 quota 文案），黑名单命中即不重试且优先于白名单与内建判定；仅对新建会话生效。重试保留已完成工具结果，不重新发送原始任务；正常结束不凭空判断“任务未完成”，仅对明确的可恢复失败重试。重试详情仅显示次数、错误和预计继续时间，不收纳正文、思考或工具调用；成功后折叠，可手动展开。新记录保存首次重试开始时的消息位置，成功、刷新或重启都不移到回答末尾；旧记录缺少位置时，服务启动利用首次等待时间和同代理完整、单调的消息时间线恢复边界并落盘，后续重启复用固定位置。时间缺失、倒退或边界同毫秒无法确定顺序时保留原来的末尾回退，不猜测归属。终态清除当前等待字段，成功清除当前错误，尝试历史仍保留；记录随会话保存，服务重启不会自动继续执行。
