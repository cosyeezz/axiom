...
输入框上沿「＋」菜单提供 Skill、工作空间文件、工作空间文件夹三个入口，使用统一线条 SVG 图标。搜索列表展示技能说明；文件/文件夹按层级浏览，过滤 `.git`、`node_modules` 与符号链接，服务端 realpath 校验禁止浏览工作空间外目录。选择后显示可移除标签，不改变原模型选择与运行信息布局。Skill 在草稿开头插入 `/skill:名称 `，每次显式加载一个；文件/文件夹以路径引用随消息发送，由代理按需读取，不上传或递归灌入整目录。标签随页面内草稿切换保留，发送成功清空，失败保留。
