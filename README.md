...
纯文本代码块中的 Unicode 框线示意图（至少两行含制表线）按字素网格显示：中文/全角及 emoji 占两格，其他字符占一格，避免系统字体回退导致边框错位；保留并排布局、原文复制和窄屏横向滚动，不转成表格。组合字符不拆开；超过 20,000 字符保留普通代码展示以限制 DOM 数量。网格不自动修复原文本身缺少/多出的空格，也不猜测图的语义。浏览器检查：`python tests/text-diagram-ui.py`（需 Playwright Chromium）。
