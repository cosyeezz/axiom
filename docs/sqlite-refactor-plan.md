...
复核更新：摘要/触发记录在崩溃窗口缺entryId已确认，恢复仅按唯一助手时间戳补齐；原V01探针绕过真实撤回限制，不能作为“污染委派”证据，同毫秒/缺时间旧记录保留不猜。query_only导致撤回SDK/网页分裂已复现修复，清理整组短事务且失败仍交还输入、保留重试；retry词表首次主代理漏传与恢复漏存亦已补回。跨进程配置读改写丢更新已复现，最终同步短事务/CAS保护另行集成。完成通知与用户输入竞态仍须按具体复现区分，不按猜测删除正常数据。
