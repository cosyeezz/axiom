...
「新会话」上方的「打开工作空间」在 Windows 服务宿主上拉起原生文件夹选择窗口，取消不切换；仍复用已有目录/会话切换和信任流程。顶栏路径后的复制、文件夹图标分别复制路径与在 Explorer 打开当前会话目录。非 Windows 可继续手动输入目录，系统打开功能目前仅支持 Windows。右上角只显示 WebSocket 连接状态，底部停止按钮为 `Stop`。新增 `workspace.pick`、`workspace.reveal({sessionId})`、`workspace.browse({sessionId,path?})`，沿用本地 WS 来源校验；不接受任意系统命令。
