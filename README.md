...
- 等待模型响应显示「连接中…」，收到思考显示「思考中…」，运行状态配动态图标；任务结束后切为静态标识，尊重系统的减少动态效果设置。
- 连续没有正文、没有工具调用的思考合并展示，思考内容仍可折叠展开，不再堆叠空白 AXIOM 卡片和重复模型用量。
- 工具调用默认只显示名称、短摘要及执行/完成/失败状态，点击或键盘展开查看详情：edit 优先显示工具返回的实际红绿 diff，没有 diff 时明确标注为请求改动。默认窄屏使用统一视图、宽屏使用左右对比；支持手动切换并在浏览器记住选择，窄屏始终回退统一视图；write 显示写入内容（无法仅凭参数判断新建或覆盖）；bash/read 等显示参数和文本输出。内容以纯文本安全展示，展开后才生成详情，每节最多显示 60,000 字符，完整结果保留在原会话记录中。
