...
旁证样本：`aff6e5c0`（588KB，即用户报告卡顿的那条会话，首条消息="目前axiom的前端页面渲染长对话会卡…"），体量远小于上表样本，不足以暴露卡顿，故不作为主样本。
...
| 场景 | 实测结论 | 是否复现 |
| --- | --- | --- |
| 打开长会话首屏 | 首次输出 1.17s，完全渲染 1.63s，含 938–955ms 单次长任务 | 复现 |
| 切换长会话 | 切换触发 1345ms 长任务（早前一轮 2431ms），点击到完成 3.1s | 复现 |
| 流式越来越卡 | 文本尾部每帧 7–12ms（头部 2–4ms），随文本增长明显劣化 | 复现（微基准口径） |
| 滚动掉帧 | 静止基线即 ~31.3ms/帧（≈32fps），滚动与静止无差异，120 帧内超 33ms 帧 0–1 个 | **未复现** |
| 内存持续增长 | 3 轮长短切换后长会话侧 23.5→23.8→24.2MB（+0.7MB），堆随 GC 回落 | **未复现（短周期）** |

折叠成本单独测量（判断「现有折叠机制是否够用」）：单个 tool-record 开/关 3–10ms；332 个全开 4.1ms 且 DOM 节点数不变 → 工具详情确为惰性渲染，折叠本身不是瓶颈。详见 `scroll-fold.json`。

## 五、限制与注意事项

1. **rAF 基线偏低**：本环境静止帧间隔即 ~31.3ms（约 32fps），说明测量环境存在 vsync/节流，绝对值不可当作 60fps 设备上的表现；场景间需看**相对差异**。
2. **流式耗时为微基准**：由「对真实文本直接调用 `renderMarkdown`」测得，非真实模型流式事件，故应视为真实流式开销的下界（未叠加 `app.js` 的 delta 处理与 DOM 重建）。
3. **内存采样受 GC 时机影响**：同一实例不同时刻 `usedJSHeapSize` 可差 20MB+，单点值不宜跨采样直接比较；短周期未观测到泄漏不等于长期无泄漏。
