...
test("inspector tabs, safe collapsible JSON and itemized billing", () => {
  ...
  try {
    ...
    tools.firstChild.open = true;
    renderTools(tools, source);
    assert.equal(tools.firstChild.open, true, "updates retain disclosure state");
    ...
    assert.match(body.textContent, /\$0.120000/);
    assert.equal(body.querySelectorAll("dt").length, 4);
