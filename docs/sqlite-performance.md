...
拆表后全面优于旧 KV 整份保存：大任务场景单次保存写入字节从 **1.35MB 降到 35B~1.3KB**（约 3~5 个数量级），启动从 **全量解析+逐会话创建 SDK（337ms、16 个 SDK、事件循环停顿 109ms）** 变为 **元数据启动（7.8ms、无待通知 0 个 SDK、停顿 4.5ms）**。硬门槛 **6/6 通过**，双进程锁实验超时可见。

## 运行命令

```bash
# 在本 worktree 根目录（F:/worktrees/Axiom-sqlite-benchmark，分支 feat/sqlite-benchmark）
# 前置：被测工作树（默认 ../Axiom-sqlite-refactor-plan）需已 npm install，
#       脚本把它的 node_modules 以 junction 接入临时快照（e2e 需要 zod 等依赖）
...
被测    F:/worktrees/Axiom-sqlite-refactor-plan 磁盘快照（src+public），读取时刻
        2026-09-13T05:22:29Z（含 A 的 notified/progress 独立列改动与 database.js
        连接前预创建/chmod 修复，src mtime：sessions 05:07Z / session-store 05:08Z / database 05:12Z）
...
|---|---|---|---|---|---|
| 改标题 | 7.0ms | 1,354,534B | 1.9ms | 69B | ~19,600× |
| 追加摘要 | 11.0ms | 1,355,188B | 1.9ms | 1,331B | ~1,000× |
| 任务进度 | 11.1ms | 1,355,579B | 2.0ms | 834B | ~1,600× |
| 任务通知(notified) | 5.0ms | 1,355,579B | 0.25ms | 58B | ~23,000× |
| 进度送达+修剪 | 10.8ms | 1,355,573B | 4.0ms | 955B | ~1,400× |
| 计时更新 | 7.2ms | 1,355,573B | 2.0ms | 35B | ~38,700× |
...
存储层启动读（大数据集）：旧 `list("sessions")` 全量解析 **104ms**；新元数据读 **0.063ms** +
待通知查询 0.010ms；单会话全投影首开读 7.9ms（按需，打开哪个读哪个）。

## 端到端（真实 Sessions + fake SDK）

| 场景 | 启动耗时 | 启动 SDK 数 | 首开 | 事件循环最大停顿 | RSS 增量 |
|---|---|---|---|---|---|
| 旧 small 无待通知 | 25.4ms | **8（全量创建）** | 启动时已含 | 3.7ms | +2MB |
| 旧 small 1 会话待通知 | 35.5ms | **8（全量创建）** | 启动时已含 | 11.3ms | +9MB |
| 旧 large 无待通知 | **337.4ms** | **16（全量创建）** | 启动时已含 | **109.4ms** | +38MB |
| 新 small 无待通知 | 1.9ms | **0（硬门槛）** | 6.2ms | 0.3ms | +1MB |
| 新 small 1 会话待通知 | 10.3ms | **1（只恢复待通知）** | 3.3ms | 1.1ms | +1MB |
| 新 large 无待通知 | **7.8ms** | **0** | 23.1ms | 4.5ms | −5MB |
...
2. **notified 不携带 runtime/result**：notified 更新在 SQL 绑定层计量，小任务 25B = 大任务
   ...
   门槛同时断言计量层观测到 SQL 写入（各 1 次），防止语句缓存绕过代理后门槛空转。
...
|---|---|---|
| 持锁 300ms | 300ms | 等待 325.8ms 后写入成功（阻塞可见、可恢复） |
| 持锁 6000ms（超过 busy_timeout） | 6000ms | 等待 5552.0ms 后失败：`database is locked`（超时可见，未吞错） |

victim 端以 busy_timeout=0 的探针连接等 holder 真正持锁后才开始计时（两子进程并发启动，
否则 victim 可能抢在持锁前写入，实测发生过），保证测量落在持锁窗口内。
...
- RSS 为子进程采样，受 GC 时机影响（新 large 端到端出现 −5MB 属正常波动）。
- 历史匹配不做平方扫描属 F 包范围，本脚本 fake `historyEntries()` 恒为空，未覆盖。
- 门4 在存储层验证（喂入记录本无 token）；真正防泄漏在 D/E 层，由其单测覆盖。
- 本次读取的 E 快照含 A 的独立列改动与 database.js 连接前预创建/chmod 修复
  （database.js mtime 2026-09-13T05:12Z）；A 最终交付后主审应以同一脚本对最终代码重跑
  （命令见上，结果可对照本报告）。
