...
- 默认配置可分别设置主/子代理供应商、模型，以及 Skills、MCP 服务、插件的全部或自定义选择（可全部取消）。保存仅影响之后创建的普通会话，不修改当前/已有会话，也不启动代理。选择即通过 WebSocket 自动保存，写入本机 Pi 用户目录下 `axiom/defaults.json`（通常 `~/.pi/agent/axiom/defaults.json`），服务重启保留，不修改 Pi 的 settings/MCP 文件。默认能力预览实时跟随选择，状态提示区区分保存中、成功和失败；失败不覆盖旧配置。
...
- 能力在创建时确定，设置弹窗显示主/子代理的能力摘要；模型仍沿用原有空闲时调整机制。未配置时子代理默认全部能力、模型跟随主代理；普通会话使用保存的默认配置，自定义会话可分别选择。状态通过会话快照恢复，服务重启丢失。
...
支持供应商、模型、思考程度选择，任务输入、停止、新会话和实时思考/回答/委派状态。支持会话列表（切换、新建、删除）和工作空间路径输入/历史目录选择，会话按目录筛选；切换不取消后台任务。已有会话目录固定，子 Agent 继承父会话目录。供应商、模型与思考选项位于发送按钮左侧，选择控件按内容宽度保持单行，桌面统一 32px 高度、手机 40px 触控高度，深灰表面与箭头；支持原生可样式化下拉菜单的浏览器使用统一暗色选项面板，其他浏览器回退原生菜单，窄屏仅选择区横向滚动，发送/停止按钮始终可见。任务输入框位于执行记录下方；Enter 发送，Ctrl/Cmd+Enter 或 Shift+Enter 换行，中文输入法选词不触发发送。Esc 优先关闭设置弹窗（不会停止任务），其次关闭手机侧栏，否则停止当前会话任务。设置支持关闭按钮、点击遮罩及原生焦点约束，保存失败在弹窗内提示并恢复原配置，运行中或断线时禁用配置。思考正文使用斜体；回答通过 marked + DOMPurify 安全渲染 Markdown，覆盖流式、最终消息与历史恢复。目录来自已认证 Pi 模型；思考选项来自当前模型实际支持的等级。主模型和思考配置应用于主会话；成功调整后记为后续新建会话的回退值（跨工作空间），普通新会话若设置了固定默认主模型则优先使用固定模型，已有会话保持原配置。仅调整子代理、切换已有会话或保存失败不会覆盖回退值，当前会话调整也不会覆盖显式保存的默认新会话配置；最近模型/思考回退值保存在服务内存中；显式默认新会话配置独立落盘，重启恢复。侧栏底部「设置」取代运行时标语，打开统一设置弹窗；左侧目前只有「默认新会话设置」，右侧顶部「会话能力」实时展示下方默认主/子 Agent 选择的供应商、模型、思考等级和 Skills/MCP/Extensions 清单，不再展示当前会话配置（仅显示技能名、扩展包名或文件名，不显示绝对路径；内部能力 ID 不变），再直接编辑默认主/子配置，选择即通过 WebSocket 自动落盘，没有保存按钮，不再打开二级弹窗。配置与自定义新会话复用同一个表单及选择器；顶部能力为默认配置预览，不代表当前会话已加载或调用。子任务可独立设置思考程度或跟随主 Agent，并由运行时按模型支持调整。子 Agent 模型按会话独立保存，切换会话、刷新或断线重连会恢复，服务重启后丢失。刷新自动连接。页面内断线重连可恢复原会话；刷新后加载服务端会话列表，可重新选择会话。服务重启仍不保留内存会话。

新增协议：`sessions.list` 返回内存会话列表；`session.create` 可传 cwd（校验目录存在）；`models.list` 返回模型目录；`session.configure` 接收 sessionId、model（provider/id）与可选 thinking、subagentModel，返回实际配置和支持等级。subagentModel 为 provider/id；null 表示跟随主 Agent，省略则保留原值；未知模型拒绝保存。运行中不可切换模型。静态资源位于 public/。
...
默认新会话配置通过同目录临时文件原子替换保存，写入串行执行；配置损坏时启动报错，不静默重置。当前运行状态、结果、会话都在内存，进程重启后丢失，不自动续跑。会话需显式关闭以释放历史与结果；长期服务需监测内存。发送缓冲过大时断开慢客户端，重新 attach 获取快照；不静默丢正文。单条 WebSocket 输入上限 1 MiB，属于网络输入保护，不是任务数量限制。
