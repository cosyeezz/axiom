...
- 验证：实施前基线 400 通过、2 跳过；变更后 npm test 共399项，397通过、2跳过、0失败（移除旧预设测试并补目录隔离回归）。索引已重建，git diff --check通过。删除配置时已有会话压缩也立即回落全局，其他目录不变；README迁移表述已校正为技能保留在所属目录。尚未提交或合并最新master。
