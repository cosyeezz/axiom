...
test("会话级预算随会话落库：全局值改了，重开旧会话仍用创建时的预算", async () => {
  ...
  try {
    ...
    assert.deepEqual(item.taskBudget, { maxTurns: 30, wrapUpWindow: 3 }, "注释承诺「运行中不热更」，重开也不得热更");
    const fresh = await sessions.create(root);
    assert.deepEqual(sessions.get(fresh).taskBudget, { maxTurns: 60, wrapUpWindow: 6 }, "新会话才跟随新全局值");
...
test("全局预算读写口径对称：脏键不丢合法值，无库实例报未启用持久化", async () => {
  ...
  try {
    ...
    assert.deepEqual(new Sessions(factory, undefined, undefined, database).getTaskBudget(), { maxTurns: 40, wrapUpWindow: 4 });
    // 真正非法的值仍回退默认，不阻断启动。
