...
- 将会话拖到列表底部的「已完成」区域即可收起；该区域默认折叠，展开后最多占侧栏 35%，避免挤占正常列表，不显示数量。
...
「新会话」上方的「打开工作空间」在 Windows 服务宿主上优先通过已安装的 PowerShell 7（`pwsh.exe`）拉起现代原生文件夹选择框，支持地址栏、搜索和快捷访问；未安装时回退到系统 PowerShell 的旧式选择框，不自动安装依赖。选择框带置顶所属窗口，避免后台启动时窗口藏在浏览器后面；选择超时（5 分钟）会释放占用并提示重试，选择后在已有同目录会话间切换或创建新会话，取消不切换；仍复用已有目录/会话切换和信任流程。顶栏标题与路径采用紧凑双行布局，与左侧菜单按钮垂直居中；路径图标固定 24px，避免通用按钮最小高度撑大行距。路径后的复制、文件夹图标分别复制路径与在 Explorer 打开当前会话目录。手输目录路径的表单已移除；`workspace.pick` 目前仅支持 Windows，其他系统暂无打开其他工作空间的入口，需要时再补。右上角只显示 WebSocket 连接状态，底部停止按钮为 `Stop`。新增 `workspace.pick`、`workspace.reveal({sessionId})`、`workspace.browse({sessionId,path?})`，沿用本地 WS 来源校验；不接受任意系统命令。
