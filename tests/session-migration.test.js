...
test("旧 defaults.json / presets.json 一次性迁入库：验证成功才标记，源文件保留，此后 JSON 非权威", async () => {
  ...
  try {
    ...
    const first = new Sessions(factory, defaultsPath);
    ...
    const second = new Sessions(factory, defaultsPath);
...
test("迁移前坏 defaults.json 只警告不标记，不阻断启动；修复后可再迁移", async () => {
  ...
  try {
    ...
    sessions.close();
    const repaired = new Sessions(factory, defaultsPath);
    await repaired.loadDefaults(); // ENOENT 静默
...
test("旧磁盘会话一次性迁入并恢复；坏文件不标记可重试；删除后不复活；库同 id 以库为准", async () => {
  ...
  try {
    ...
    const again = new Sessions(factory, undefined, storage);
    const warnAgain = [];
    ...
    const third = new Sessions(factory, undefined, storage);
    await third.load();
    ...
    const fourth = new Sessions(factory, undefined, storage);
    await fourth.load();
...
test("库记录的历史文件缺失：跳过恢复并保留库记录原状，不让 SDK 用空历史覆盖", async () => {
  ...
  try {
    ...
    // 明确警告；不创建空历史会话，也不在原路径写出新文件
    assert.deepEqual(sessions.list(), []);
    assert.ok(warnings.some((message) => message.includes("ghost") && message.includes("缺失") && message.includes(ghostFile)));
...
test("全局摘要设置：默认 3/6/30，configure 校验后持久化，重启恢复最新值，坏记录回退默认", async () => {
  ...
  try {
    let sessions = new Sessions(factory, join(dir, "defaults.json"));
