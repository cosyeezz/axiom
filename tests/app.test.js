...
test("page preserves drafts, recovers failed connections and paints tasks on demand", async () => {
  ...
  try {
    ...
    await settle();
    await settle();
    assert.equal(requests.filter((req) => req.type === "session.defaults.list").length, 0, "全局默认页不再列举工作空间");
    assert.equal($("defaults-editor").contains($("create-form")), true);
    assert.match($("create-capabilities").textContent, /\[全局\] Skill A/);
    assert.match($("create-capabilities").textContent, /\[当前项目\] Skill B/);
    assert.equal($("defaults-scope-label").textContent, "全局默认", "默认编辑全局默认配置");
    ...
    await settle();
    // 配置入口固定作用域，不再列举工作空间。
    const configuredPaths = () => $("defaults-scope-label").textContent;
    ...
    $("settings").close();
    const workspaceGroup = row("a").closest(".workspace-group");
    const wasOpen = workspaceGroup.open;
    const workspaceConfig = workspaceGroup.querySelector(".workspace-header .workspace-config-btn");
    assert.ok(workspaceConfig, "侧栏工作空间右侧提供配置入口");
    ...
    await settle();
    assert.equal(lastDefaults.cwd, workspace, "保存当前工作空间配置带上 cwd");
    assert.equal(directoryDefaults.get(workspace).thinking, "off");
    assert.deepEqual(defaults, globalDefaultsBefore, "目录配置不写全局默认");
    assert.equal(window.sessionState().sessionId, "a", "保存目录配置不切换会话");
    assert.equal($("defaults-workspace"), null, "不提供工作空间枚举下拉");
    // 删除目录配置：回落全局默认，当前会话不动。
    $("defaults-delete").click();
    await settle();
    assert.equal(lastDefaultsDelete.cwd, workspace, "恢复的是当前工作空间");
    assert.equal(directoryDefaults.has(workspace), false);
    assert.equal($("create-main-model").value, "other/child", "编辑器回到全局默认配置");
    assert.equal($("defaults-delete").hidden, false, "仍在工作空间作用域");
    assert.equal(sessionsSwitched(), switchedBefore, "删除目录配置不切换会话");
    $("settings").close();
    const defaultsBeforeSession = structuredClone(defaults);
    const sessionMenu = row("a").querySelector(".session-options");
    sessionMenu.open = true;
    const sessionConfig = sessionMenu.querySelector(".session-configure");
    assert.ok(sessionConfig, "已有会话下拉包含配置项");
    sessionConfig.click();
    await settle();
    ...
    $("create-main-thinking").dispatchEvent(new window.Event("change", { bubbles: true }));
    await settle();
    assert.deepEqual(defaults, defaultsBeforeSession, "当前会话设置不得改写全局默认");
    assert.equal(requests.filter((req) => req.type === "session.configure").at(-1).sessionId, "a");
    $("settings").close();
    row("b").querySelector(".session-configure").click();
    await settle();
    assert.equal(window.sessionState().sessionId, "b", "非当前会话的配置入口先切换到目标会话");
    assert.equal($("defaults-scope-label").textContent, "仅当前会话");
    $("settings").close();
    row("a").querySelector(".session-item").click();
    await settle();
    ...
    const composer = task.querySelector(".task-composer");
    const taskInput = task.querySelector(".task-input");
    assert.equal(task.querySelector(".task-body").contains(composer), false, "composer stays outside the scrolling transcript");
    taskInput.value = "继续检查边界";
    composer.dispatchEvent(new window.Event("submit", { bubbles: true, cancelable: true }));
    await new Promise(setImmediate);
    const appended = requests.filter(req => req.type === "task.append").at(-1);
    assert.equal(appended.taskId, "child");
    assert.equal(appended.text, "继续检查边界");
    assert.equal(appended.mode, "steer");
    assert.equal(taskInput.value, "");
    task.querySelector(".task-message-mode").value = "followUp";
    taskInput.value = "下一轮再处理";
    composer.dispatchEvent(new window.Event("submit", { bubbles: true, cancelable: true }));
    await new Promise(setImmediate);
    assert.equal(requests.filter(req => req.type === "task.append").at(-1).mode, "followUp");
    const taskModel = task.querySelector(".task-model");
    taskModel.value = "other/child";
    taskModel.dispatchEvent(new window.Event("change"));
    await new Promise(setImmediate);
    assert.equal(requests.filter(req => req.type === "session.configure").at(-1).subagentModel, "other/child");
    assert.match(task.querySelector(".task-composer-feedback").textContent, /下一次模型请求/);
...
test("compaction settings edit per scope and fold transcripts in place", async () => {
  ...
  try {
    ...
    assert.equal(scopedCompaction().querySelectorAll("input[type=number]")[0].value, "60000", "未配置的当前目录继承全局阈值");
    assert.equal($("defaults-delete").hidden, false, "目录配置可以删除");
    ...
    await settle();
    assert.equal(lastDefaults.cwd, state.cwd, "目录配置保存带上 cwd");
    assert.equal(directoryDefaults.get(state.cwd).compaction.tokenThreshold, 40000);
    assert.deepEqual(defaults.compaction, globalCompaction, "目录配置不污染全局默认");
    $("defaults-delete").click();
    await settle();
    assert.equal(lastDefaultsDelete.cwd, state.cwd, "恢复当前目录的全局默认");
    assert.equal(directoryDefaults.has(state.cwd), false);
