...
export function mountComposerControls({ state, providers, models, levels, selectModel }) {
  ...
  split.append(primary, arrow); actions.append(trigger, split);
  ...
  function close(panel, focus = true) { if (panel.matches(':popover-open')) panel.hidePopover(); if (focus) focusPrompt(); }
  ...
  function column(title, entries, selected, choose) {
    ...
    function render() {
      ...
      for (const [value, label] of entries.filter((entry) => entry.join(' ').toLowerCase().includes(search.value.toLowerCase()))) {
        const item = document.createElement('button'); item.type = 'button';
        const text = document.createElement('span'); text.textContent = label;
        item.append(text, icon(value === selected ? 'check' : 'next'));
        item.title = label; item.setAttribute('aria-pressed', String(value === selected));
        item.onclick = () => choose(value); list.append(item);
      }
    ...
    if (title !== '供应商') {
      ...
      back.onclick = () => { if (title === '思考等级') model = null; else { provider = null; model = null; } showModels(); modelPanel.lastElementChild.querySelector('input').focus(); };
      section.append(back);
    }
    search.oninput = render; render(); section.append(heading, search, list); return section;
  }
  function showModels() {
    modelPanel.replaceChildren(column('供应商', providers(), provider, (value) => { provider = value; model = null; showModels(); modelPanel.children[1]?.querySelector('input')?.focus(); }));
    if (provider) modelPanel.append(column('模型', models(provider), model, (value) => { model = value; showModels(); modelPanel.children[2]?.querySelector('input')?.focus(); }));
    if (model) modelPanel.append(column('思考等级', levels(model).map((value) => [value, value]), state().thinking, async (value) => {
      close(modelPanel); await selectModel(model, value); if (typeof document !== 'undefined' && document?.body) { refresh(); focusPrompt(); }
    }));
    position(modelPanel, trigger);
  }
  trigger.onclick = () => {
  ...
  function refresh() {
    ...
    const modelName = document.createElement('span'); modelName.textContent = current.model?.split('/').slice(1).join('/') || current.model || '选择模型';
