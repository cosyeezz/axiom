...
卸载使用实际包名 `@myworkbench/axiom`，不是安装来源 `github:cosyeezz/axiom`。当前没有统一卸载命令，先取消自启、停止服务，再卸载包。
...
- 会话行只保留标题与「⋯」入口，点击后在三点右侧显示独立浮动菜单，可标记已完成/移回待继续、新页签打开、重命名、删除；窄屏自动向内避让，不挤动下方标题；Esc 或点击外部关闭。只有明确标记完成才进入已完成，标记不会停止任务。搜索仅筛选当前工作空间。
