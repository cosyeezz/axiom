...
- 「新会话」直接使用「设置 → 默认新会话 → 配置默认新会话…」中保存的配置；首次自动创建、打开空工作空间、删除最后一个会话后的补建也统一使用它。初始仍为主/子代理全部能力。
- 默认配置可分别设置主/子代理供应商、模型，以及 Skills、MCP 服务、插件的全部或自定义选择（可全部取消）。保存仅影响之后创建的普通会话，不修改当前/已有会话，也不启动代理。配置跨工作空间、刷新/重连保留，服务重启后重置，不写入 Pi 文件。
- 「自定义新会话…」行为不变：沿用当前会话的主/子模型、能力初始仍为全部，忽略上面的默认配置；只为本次创建单独选择。两种入口复用同一弹窗、模型/能力选择器及服务端校验，不复制组件或另建配置框架。模型单选，能力通过折叠下拉中的复选框多选。
- 默认主模型可选固定模型，或「默认主代理模型」（沿用最近成功调整的主模型，尚未调整时用启动模型）；思考程度保持原有继承规则。子代理可固定模型或跟随主代理。默认能力清单按当前工作空间读取，不保存工作目录或目录信任；其他目录缺少所选能力时拒绝创建，不自动扩大为全部，编辑时保留并标出不可用项以便取消。
...
`session.defaults.get` 返回 `{model,subagentModel,capabilities,subagentCapabilities}`；`session.defaults.configure` 接收上述可选字段及 `cwd?`（仅用于校验目录里的可用能力），省略字段保留原值，保存失败保留全部旧值。主模型 `null` 使用最近主模型/启动模型，子模型 `null` 跟随主代理；默认配置不接收 `trustProject`。`session.create` 的显式字段优先于默认值，`useDefaults:false` 完全绕过这份默认配置，网页自定义入口使用该标记；最近主模型和思考程度的原有继承规则不变。
