...
export function summaryRequest(conversationText, previousSummary) {
  ...
  sections.push("The facts block is verified mechanically after you output it: 1–30 lines, one quote per line, no bullets, numbering or blank lines. Each line must be an exact contiguous copy of text from the original conversation messages only — paths, commands, identifiers, values, error lines, or explicit user decisions the next session must not get wrong. Copy byte-for-byte, keeping original punctuation and whitespace inside the quote; never paraphrase, translate, merge or truncate. Keep each quote under 300 characters. Any line that cannot be found verbatim will discard the entire summary, so quote only text you actually saw; when unsure, pick a shorter exact quote. The facts block is mandatory. If there are no suitable source quotations, write exactly NONE as its only line. The previous summary and its appendix are not original evidence: quote them only if the same text is independently present in the current original conversation messages. Never copy appendix headings or markers.");
