...
### 2026-09-22 答复边界决定标题提取与完成卡展示
- 症状：过程说明与 `<axiom_display>` 开标签同行时，说明和标签进入完成卡；答复首行的 `<title>` 漏出，目标轮次小结也可能以标题标签开头。
- 根因：解析器只认独占行标签；记忆标签处理早于正式答复拆分，使块内首行不再是整条回复的首行。
- 修复：public/answer-tags.js 识别说明行尾开标签及流式残片；public/app.js、src/goal.js 先拆答复再过滤记忆标签；src/session-memory.js 从正式答复提取会话标题。
- 防再犯：同时覆盖实时与历史渲染、标题定案、目标小结及流式半截开标签；保留代码示例中标签原样展示。

### 2026-09-22 压缩摘要与任务状态必须同版本
- 根因：将来源标签格式要求注入原生摘要尤其split-turn会制造额外失败点；失败回退又绕过原有校验。
- 修复：原生摘要提示不改，独立状态只在压缩冻结输入上维护；summary与details.stateDoc同条日志提交，context从各自分支派生，不新增权威文件。
- 防再犯：状态失败/取消/超窗不得部分提交或沿用旧状态冒充更新；浏览器fixture要同步stateDoc。连续两轮要覆盖缺usage的恢复消息（SDK取usage可能抛错），保留保守窗口估算。

### 2026-09-22 子任务停止总结不能被当成空闲
- 症状：增加 wrapping/stopping/summarizing 后，仅识别 starting/running 的会话列表、复制、Goal 和服务停止守卫提前放行。
- 根因：状态机扩展未同步所有活动状态消费者；Promise.race 超时又不等于底层工作已停止。
- 修复：sessions/server 使用 task-execution.js 活动状态集，Goal/UI 同步五态；停止有界等待并如实记录 unconfirmed，未确认时禁止共享会话总结或续接。
- 防再犯：新增阶段时连同守卫、快照和重连一起测；每次执行标识隔离迟到回调，交付前保留停止事实，不把成功总结视为任务完成。
